# Docs Map

This folder is the human-facing map of the project: setup, demos, design notes, and session records.

## Start Here

- [lm-studio-mcp-guide.md](lm-studio-mcp-guide.md): LM Studio + MCP setup and usage.
- [mcp-chat-playbooks.md](mcp-chat-playbooks.md): copy/paste chat playbooks for deterministic tool use.
- [index.html](index.html): default docs landing page with project intro and primary demo links.
- [docs-hub.html](docs-hub.html): styled docs hub page.

## Live Demo Artifacts

- [hospital-cpm-playbook-session.html](hospital-cpm-playbook-session.html): CPM-style hospital control-room transcript.
- [fantasy-overlord-session.html](fantasy-overlord-session.html): multi-character simulation transcript with Prolog console view.
- [fantasy-overlord-session.md](fantasy-overlord-session.md): markdown version of the fantasy transcript.
- [examples/](examples/): canonical captured transcript JSON/MD/HTML artifacts.

## Walkthroughs

- [family-tree-walkthrough.md](family-tree-walkthrough.md): deterministic logic tool walkthrough.
- [family-tree-agent-mcp-walkthrough.md](family-tree-agent-mcp-walkthrough.md): LLM + MCP family-tree walkthrough.
- [rule-table-walkthrough.md](rule-table-walkthrough.md): logic-table derivation walkthrough.
- [rule-table-agent-mcp-walkthrough.md](rule-table-agent-mcp-walkthrough.md): LLM + MCP rule-table walkthrough.
- [drug-triage-walkthrough.md](drug-triage-walkthrough.md): deterministic triage logic walkthrough.
- [drug-triage-agent-mcp-walkthrough.md](drug-triage-agent-mcp-walkthrough.md): LLM + MCP triage walkthrough.
- [fantasy-overlord-mcp-walkthrough.md](fantasy-overlord-mcp-walkthrough.md): pause/resume simulation with multi-hop inference.
- [constraint-editor-mvp-playbook.md](constraint-editor-mvp-playbook.md): constraint graphics MVP playbook.

## Core Design Docs

- [semantic-grounding.md](semantic-grounding.md): NL -> symbolic grounding approach.
- [failure-explanations.md](failure-explanations.md): explainable failure model and taxonomy.
- [fact-intake-pipeline.md](fact-intake-pipeline.md): intake architecture and routing.
- [write-path-spec.md](write-path-spec.md): mutation/write semantics contract.
- [memory-ingestion-and-revision-notes.md](memory-ingestion-and-revision-notes.md): ingestion/revision notes.
- [pre-thinker-control-plane.md](pre-thinker-control-plane.md): stateless pre-thinker control-plane direction.
- [ontology-context-routing-spec.md](ontology-context-routing-spec.md): context routing spec (secondary track).

## Scenario & Positioning

- [uses-and-scenarios.md](uses-and-scenarios.md): where the logic layer is useful.
- [stretch-logic-scenarios.md](stretch-logic-scenarios.md): larger and more ambitious scenario ideas.
- [agent-integration.md](agent-integration.md): agent-facing integration model.
- [agent-ingestion-tests.md](agent-ingestion-tests.md): ingestion behavior test prompts.

## Research / Narrative

- [neuro-symbolic-2026-landscape.md](neuro-symbolic-2026-landscape.md): landscape review framing.
- [neuro-symbolic-2026-stack-pitch.md](neuro-symbolic-2026-stack-pitch.md): implementation pitch framing.
- [lmstudio-classifier-matrix.md](lmstudio-classifier-matrix.md): local-model classifier matrix findings.

## Session Records

- [sessions/](sessions/): long-form session notes and part files.
- [sessions/README.md](sessions/README.md): sessions folder note.

## Misc

- [diagrams/](diagrams/): diagram source notes and visual assets.
